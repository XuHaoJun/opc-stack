...
- **preview port 預設只綁 `127.0.0.1`** — 遠端操作這台機器 (Tailscale / SSH) 時 `DEV_URL` 會指向連不到的位址。設 `.env` 的 `DEVENV_HTTP_BIND` **與** `DEVENV_HTTP_PUBLIC_HOST` 為 tailnet IP (兩個都要, 否則 URL 與實際綁定不一致), 然後 recreate paperclip 容器。用 tailnet IP 而不是 `0.0.0.0`: prototype dev server **沒有任何認證**, 綁 0.0.0.0 等於對整個 LAN 開放。
