...
- **compose 的 `podenv` service 只多四項宣告, 每項都是量出來的必要, 不是預防性加上去的**: `security_opt: [seccomp=unconfined]` (沒有它 rootless podman 直接起不來, `cannot clone: Operation not permitted`)、`devices: [/dev/net/tun]` (pasta 需要)、`mem_limit` (唯一有效的記憶體旋鈕, 見上一條)、`init: true` (podman 自己當 PID 1 不會 reap 被重新父代的子行程 —— 量到 5 次 nested run 留下 10 個 zombie)。同樣值得記下**不需要**的東西, 免得有人「順手」加上去: 不用 `apparmor=unconfined` (這台機器沒載 AppArmor)、不用 `label=disable` (沒有 SELinux)、不用 `/dev/fuse` (kernel 7.2 原生支援 rootless overlay)、不用 `privileged`、不用任何 host mount。
