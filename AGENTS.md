...
- **`upstream/`** = 4 個 git submodule, pin 在 upstream tag commit (乾淨 checkout, 永不直接編輯): buzz `desktop-v0.5.14` / hermes `v2026.8.19` / paperclip `v2026.817.0` / tencentdb `v2.0.0` —— **這行會過期** (已經過期過一次: 它一度還寫著 hermes `v2026.8.16` 與 paperclip 的 canary tag), 真正的 pin 與「有沒有更新的 stable tag」一律跑 `scripts/outdated.sh` 問。
...
- **devenv 第四類後端 RabbitMQ** (`devenv-rabbitmq`): shared RabbitMQ `4.3.5-management-alpine`，每個 opt-in lease (`--with rabbitmq`) 建一個 `devenv_<key>` vhost + 同名、無 management tag 的 user，寫出 `AMQP_URL`。隔離是 logical，不是 CPU/RAM/disk/failure-domain 隔離；每個 vhost 固定套 `max-connections` / `max-queues`。management API 只在 compose network 內供 provider reconcile，host 只把 AMQP 發佈到 `127.0.0.1:${DEVENV_RABBITMQ_PORT:-5673}`。**`hostname: devenv-rabbitmq` 不可拿掉**：RabbitMQ 的 on-disk DB 以 node name 分目錄，docker 預設 container-id hostname 會讓 `--force-recreate` 對同一 volume 開出一顆看似全空的新 node。`tests/devenv-rabbitmq.sh` 釘住 vhost/user 隔離、limits、AMQP publish/get、recreate durability 與 release。
- **podenv** (`docker-compose.yml` 的 `podenv` service): devenv 服務不了的兩類 daemon 的租約 —— 拒絕 multi-tenant 的 daemon、與只以 image 存在的超舊版本 (MySQL 5.7、Postgres 9.6…)。機制是 rootless podman 巢狀跑在 `podenv` 容器裡 (不是 docker-in-docker), paperclip 用 apt 裝的 `podman-remote` client 連過去。**先試 devenv** —— CLI 對 postgres/pgvector/valkey/redis/rabbitmq/S3 家族的 image 一律拒絕並印出對應的 devenv 指令。不變量 6 (無 host mount、無 privileged) 兩條都完好, 但多授予兩項 (`security_opt: seccomp=unconfined` 讓 rootless podman clone 得動、`/dev/net/tun` 讓 pasta 網路能跑), 由 `tests/podenv.sh` 的結構檢查釘住。設計見 `docs/superpowers/specs/2026-08-22-podenv-nested-container-lease-design.md`。
- 服務 (port): buzz relay 3000 (+pg/redis/minio) · frontdoor (buzz-acp→`hermes acp`, 與 buzz 共用 netns) · hermes gateway 8642 (API server; dashboard 關閉; **專家 agent 的宿主** — multiplex 服務 `hermes-profiles` volume 上的每個 profile, 目前有 `agt-scientist`) · hermes-dashboard 9119 (web UI, 掛 frontdoor 的 hermes home, 看 buzz 對話 session/thinking log) · paperclip 3100 · tencentdb core 8420 / panel 8125 / knowledge 8424 / proxy 8096 · devenv-s3 9002 · devenv-rabbitmq 5673 · devenv-pg 5433 · devenv-valkey 6380 (devenv host ports 全部 127.0.0.1 only；RabbitMQ management 15672 不發佈)。
- LLM: OpenCode Go (`https://opencode.ai/zen/go/v1`)。`.env` 填 `OPENAI_API_KEY` 一個 key 全棧通用;Hermes custom provider runtime 讀 `OPENAI_API_KEY` + `OPENAI_BASE_URL`。shared gateway/dashboard/Paperclip/TencentDB 的 model 用 `OPENAI_MODEL`（預設 deepseek-v4.1-flash）,frontdoor relay 用 `BUZZ_AGENT_MODEL`（預設 deepseek-v4.1-flash）;模型選擇以 `config.yaml` 為 source of truth。
...
只有真的從空狀態走一次 `setup.sh` 之後七條 gate 直接綠 —— 但**不要在這台上
...
做完整排練, 再從 clone 裡跑 audit-bootstrap + connectivity + scientist + podenv + devenv-s3 + devenv-rabbitmq。
...
scripts/outdated.sh           # upstream/ 四個 pin 有沒有更新的 stable tag (exit 0 全新 / 1 有更新 / 2 報告不完整)
scripts/upgrade-preflight.sh <proj> <tag>  # 升版前唯讀檢查 (exit 0 乾淨 / 1 有事要人判斷 / 2 報告不出來)
scripts/upgrade.sh <proj> <tag>  # 升版: preflight → checkout → prepare → rebuild → force-recreate → 跑 migration gate
tests/migrations.sh [proj|all]   # 第七條 gate: migration 有沒有真的前進 (連通性綠不代表這條綠)
...
docker compose exec -u node paperclip devenv provision demo --with rabbitmq       # opt-in RabbitMQ vhost/user lease
tests/devenv-rabbitmq.sh # RabbitMQ provider 結構 + live gate (AMQP, vhost isolation, limits, durability, release)
...
4. **mise toolchain 持久化**: buzz/frontdoor/hermes/paperclip 各有 `*-mise:/opt/mise` volume; 空狀態首次開機 `opc-mise-seed.sh` 自動裝 node@lts + rust@stable + omp prebuilt (`github:can1357/oh-my-pi@18.0.11`, ~170MB)。PATH 尾掛 mise shims (baked node 優先, mise 補缺口)。日常加 toolchain = `docker exec <c> mise install <tool>`; 升 omp = `mise use -g github:can1357/oh-my-pi@<ver>` **每個容器各一次** (零 rebuild; entrypoint 的檢查是 install-if-missing 不是 reconcile, 所以 bump pin 只影響空 volume)。
...
- **omp 的 `opencode-go` provider 憑證是 `OPENCODE_API_KEY`, 不是 `OPENAI_API_KEY`** —— 所以跑 omp 的每個 service (buzz/frontdoor/hermes/hermes-dashboard/paperclip) 都把它從那唯一一把 stack key 派生 (`OPENCODE_API_KEY: ${OPENAI_API_KEY:-}`)。這是**唯一**為 omp 保留的 `OPENCODE_*` 名字, 不是可清理的 legacy alias: 2026-08-20 的 env rename 移除它時假設「provider slug 不需要 env var」, 2026-09-11 實測推翻 —— 缺它時 `omp -p --model opencode-go/<任何模型>` 一律死在 `No API key found for opencode-go` (deepseek-v4-flash 與 deepseek-v4.1-flash 行為相同, 所以與模型無關), 補上它 `deepseek-v4.1-flash` 立刻正常。受影響的是 paperclip 的 agent run (`omp acp --yolo` 的第一次模型呼叫) 與任何 agent 從 terminal 叫 omp 的地方; 沒有任何 entrypoint 或 adapterConfig 會提供它。
...
- **podman `start` 對一個已死但記錄成 `running` 的容器會靜靜成功**: podenv 服務重啟後, podman 的紀錄仍寫著 `Up`, 但行程早就不在了。`podman ps --sync` **不會**reconcile 這個狀態。三個選項裡量到的行為是: `start` 對這種容器靜靜回成功 (三者中最壞的一個 —— 看起來全綠, 實際上沒人在跑); `restart -t 0` 大聲失敗 (`conmon exited prematurely`), 是三者中唯一誠實的; 只有先 `stop` 再 `start` 能真的復活它。根因: `/run/user/1000` 沒有專屬 mount, 活在容器自己的 writable overlay layer 裡, 而 `docker restart` **保留**這一層 (`--force-recreate` 才會建全新層) —— 開發時用 `--force-recreate` 測起來「看起來正常」, 正好是那條路徑把這個 bug 蓋住了。因此開機 restore 與 `provision` 一律**先 probe 再動作, 不信任記錄狀態**。**根因已修 (2026-09-11)**: `/run/user/1000` 現在是 compose 宣告的 tmpfs (見下一條的第五項), 這才是 `XDG_RUNTIME_DIR` 本來該有的生命週期, podman 因此每次啟動都做它自己的 new-boot refresh 而不是繼承上一個 instance 的 runtime state。**但 probe-first 不可以拿掉** —— `patches/paperclip/podenv/podenv` 的 on-demand 分支跑的是同一段邏輯, 而那是在活著的 stack 上, tmpfs 對「某個租約的行程還在不在」一句話都沒說。
- **一個誠實的 probe 不能是裸 TCP connect, 也不能信 `head` 的 exit code**: pasta 的 port-forwarder 會完成 TCP handshake, 即使後面沒有東西在聽 —— 裸 connect 測不出「沒人在跑」。而 `head` 在乾淨的 zero-byte EOF 時 exit 0, 曾讓 probe 有約 15% 的機率回報「活著」但其實什麼都沒收到。現在的 probe 改成**計數實際收到的 bytes**, 不看 TCP handshake 是否成功、也不看 `head` 的退出碼。
- **`podman run --memory` 在這個拓撲下被靜靜忽略**: 量到 `memory.max` 停在 `max`, 零警告。量到兩個共因 —— `/sys/fs/cgroup` 是 ro mount (cgroup delegation 沒有 host mount 做不到, 而不變量 6 禁止 host mount), 以及 podenv image 自帶的 `containers.conf` 設 `cgroups="disabled"` —— 但**沒有隔離出哪個是真正生效的成因, 不要宣稱只有一個**。唯一有效的旋鈕是整條 lane 共用的 compose `mem_limit` (`PODENV_MEM_LIMIT`)。驗證這件事**必須讀 podenv 自己的 cgroup 檔**: nested 容器裡讀到的永遠是 `max`, 在錯的地方斷言會讓測試綠得沒有意義。
- **nested 容器的網路只有 pasta 與 host netns 兩條活路**: pasta 需要 `/dev/net/tun` device, 換來 `-p` port 重映射; host netns 不需要那個裝置, 但**`-p` 會被靜靜丟掉, 不會警告**。bridge/netavark 整條路不可能 —— 它要寫 `/proc/sys`, 而 docker 把它 mount 成 ro。上游 image 的 `containers.conf` 預設 `netns="host"`, 我們的 override **只改這一行**成 pasta; `userns="host"` 刻意留著不動, 因為那是讓 `/prototypes` 底下建出的檔案歸屬停在 uid 1000 而不是某個 subuid 的原因。
- **compose 的 `podenv` service 只多五項宣告, 每項都是量出來的必要, 不是預防性加上去的**: `security_opt: [seccomp=unconfined]` (沒有它 rootless podman 直接起不來, `cannot clone: Operation not permitted`)、`devices: [/dev/net/tun]` (pasta 需要)、`mem_limit` (唯一有效的記憶體旋鈕, 見上一條)、`init: true` (podman 自己當 PID 1 不會 reap 被重新父代的子行程 —— 量到 5 次 nested run 留下 10 個 zombie)、**`tmpfs: [/run/user/1000:mode=0700,uid=1000,gid=1000]`** (前四項都是權限, 這項是**生命週期**: `XDG_RUNTIME_DIR` 的定義就是 session 結束即消失, 主機上由 systemd-logind 給 tmpfs, 容器裡沒人給。少了它 podman 的 boot ID 快取活在 overlay layer 而 `docker restart` 保留那層, 於是**每次 host 重開機 podenv 都會永久 crashloop**, 錯誤是 `current system boot ID differs from cached boot ID`, 而且只有 `--force-recreate` 或手動刪目錄能解。一行就重現: 改壞 `/run/user/1000/libpod/tmp/alive` 再 `docker compose restart podenv`。量過: `noexec` 不影響 conmon/pasta, 全新 provision 正常; 12K idle / 116K 兩個租約, 所以**不設 size cap** —— 沒有東西會無上界成長, 而一個猜出來的 cap 只會變成沒人看得懂的 ENOSPC)。同樣值得記下**不需要**的東西, 免得有人「順手」加上去: 不用 `apparmor=unconfined` (這台機器沒載 AppArmor)、不用 `label=disable` (沒有 SELinux)、不用 `/dev/fuse` (kernel 7.2 原生支援 rootless overlay)、不用 `privileged`、不用任何 host mount。
...
- **記憶入境的邊界是協定, 不是文字 (`_meta.buzz.memoryEvents`)**: 前門的 prompt 是**未 escape 的文字** —— `format_event_block` 把 event content 原樣嵌進去 (`queue.rs:1323`), `escape_semantic_text` 在整個 crate 只有 channel metadata 一個 call site, 所以 sender 可以自己寫出 `</buzz-event>` 與 `From: … hex: …`。因此被動記憶 capture 的判斷**只讀 ACP block 的 `_meta`**, 而那條路徑要三個地方同時對: `patches/buzz/patches/opc-memory-ingress-meta.patch` (buzz-acp 產生, `crates/buzz-acp/src/{queue,pool,acp}.rs`) → `patches/buzz/patches/hermes-acp-memory-ingress.patch` (hermes 原樣搬運 `field_meta`, 不讀內容) → `patches/{buzz,hermes}/memory_tencentdb/{ingress,recall}.py` (政策)。契約凍結在 `tests/fixtures/buzz-acp-prompt-blocks.json`, 兩邊各自被 gate 釘住 (`tests/memory-ingress.sh` 離線、`tests/memory-ingress-meta.sh` 跑 Rust、`tests/memory-scope.sh` live)。**改 wire 形狀要三個 patch 一起改**, 只改一邊的症狀是 capture 全關 (每個 turn 一列 `no-memory-events`), 不是報錯。spec: `docs/superpowers/specs/2026-09-11-memory-ingress-structural-provenance.md` (它取代了 2026-09-10 那份的 §7.1 機制)
## 檔案地圖
...
- `patches/tencentdb-agent-memory/MemoryCore/` — tencentdb-core 的 opc Dockerfile + `opc-tencentdb-provision.sh` (meta-plane bootstrap) + `patches/v3-meta-schemas.patch` (schema overlay: team/create + agent/create 接受顯式 id)。**這個 overlay 是 patch 不是整檔複本, 而且不可以改回去**: 它原本是一份凍結在 v2.0.0 的 448 行複本, 由 Dockerfile 無條件 COPY 覆蓋上游同名檔 —— 也就是每次升版都會**把上游自己的新增靜靜還原**。v2.0.1 已經量到後果: 少掉 `userCreateWithKeySchema` 而 router 仍然 bind 它 (`bind()` 是 lazy deref), 於是 gateway 正常開機、`/health` 綠、54 條路由 53 條對, 只有 `/v3/meta/user/create-with-key` 被呼叫時 500; 同時 `fixedAssetListWithDetail.asset_types` 被 zod strip 掉, 過濾條件無聲失效。改成 `patch -p1 --fuzz=0` 之後, 上游動到 hunk context 就是 **build 停住**
...
- `patches/paperclip/devenv/` — `devenv` CLI + `providers/{postgres,valkey,rabbitmq,s3}.sh` + `bootstrap.sql`;
  image 內落在 `/usr/local/lib/devenv/`, symlink 到 `/usr/local/bin/devenv`。schema 由 `opc-devenv-seed.sh` 每次開機套用 (冪等, 後端不通只警告不擋 `up`)。`providers/s3.sh` 用 pinned `mc` reconcile bucket/IAM/policy；`providers/rabbitmq.sh` 用 internal management API reconcile vhost/user/permissions/limits，Paperclip image 內的 `amqp-tools` 是 live/client surface。
...
- `scripts/prepare.sh` — 除了同步 patches, 還有**防漂移檢查**: 兩份 `paperclip-api` SKILL.md 與兩份 `SOUL.md` 必須逐字相同, 不同就中止 build (這條規則在本 repo 已經默默壞過三次); 另外兩個 hermes entrypoint 的 `OPC_CONFIG_VERSION` 必須等於上游 `config_defaults.py` 的 `_config_version` default —— 不等就中止, 並**點名中間的 `_migrate_to_N` 要你先讀過再 bump**。理由是 hermes 每次開機都會就地 migrate 我們 seed 的那個檔, 所以 seed 落後 = 每台乾淨安裝都在無人值守地跑沒人審過的 ladder (實際落後過 4 階: 34 vs 38)。`OPC_CONFIG_VERSION_LEGACY=34` 是**另一個常數, 凍結不動** —— 它只套在「已存在但完全沒有版本欄」的舊檔上, 讓那種檔從它當年真正對應的那一階重新入梯; 把兩者合成一個就是這次落後的成因
- `scripts/` — 操作用的一次性工具: setup / prepare / upgrade / upgrade-preflight (升版前把**每一條機械檢查**跑一遍再交給人判斷 —— 會咬人的那條 tencentdb overlay 檢查, 原本只以 skill 文件裡的一行存在, 而文件裡的一行不是檢查; `scripts/upgrade.sh` 開頭會自己跑它, findings 未經 `OPC_UPGRADE_ACK_FINDINGS=1` 確認就不動任何東西) / outdated (`npm outdated` 對 `upstream/`: **stable 家族從當下的 pin 推導**, 不是寫死的表 —— 前綴 + 純數字分量, 所以 paperclip 的 1118 個 `canary/` tag、buzz 的 12 個 tag 家族都自動落選, 而 hermes 的 4 分量 hotfix 形狀仍然合格; 讀 pin 用 `git tag --points-at HEAD` 而**不是** `git describe`, 後者把 buzz 報成 `mobile-v0.11.0-rc.2-22-g…`) / load-env (`.env` 讀取器, 被 `scripts/setup.sh`、`scripts/upgrade.sh` 與 `tests/*.sh` 共用); `host-sync.sh` (通用 host→volume 鏡像 CLI) + `host-sync-worker.sh` (容器側 engine, 唯一邏輯) + `hooks/` (per-source 轉換, ssh/gitconfig/claude-cred) + `sync-gh-creds.sh` / `sync-claude-creds.sh` (場景薄 wrapper); compose `host-sync` (gh) 與 `host-sync-claude` (Claude cred) 兩個 one-shot 每次 up 自動跑
- `tests/` — 測試與驗證腳本的慣例落腳處, 與 `scripts/` 分工: `scripts/` 是操作工具, `tests/` 是測試與驗證。移進來的檔案把冗餘的 `test-` 前綴拿掉 (目錄名已經說明性質了) —— 例外是 `tests/audit-bootstrap.sh` (本來就沒有前綴) 與 `tests/acp-smoke-test.mjs` (`-test` 是複合名稱的一部分而非前綴, 且它在 paperclip 容器內跑, 名稱與位置無關)。`tests/set-buzz-agent-owner.sh` 測的是 `scripts/set-buzz-agent-owner.sh` — 同名分屬 `scripts/`/`tests/` 兩個目錄是刻意的配對, 不是巧合, 其餘整合測試同理。gate: `tests/audit-bootstrap.sh` (靜態: 每份狀態是否都有無人值守的產生者)、`tests/connectivity.sh` (live: 既有功能沒被弄壞)、`tests/migrations.sh` (live: 升版後 migration 有沒有真的前進 —— **這條存在的理由是 connectivity 在每一次無聲升版失敗裡都是綠的**: config ladder 吃掉 frontdoor 的 `system_prompt` 那次, 容器綠了 27 小時; 它比對 buzz `_sqlx_migrations` / paperclip `drizzle.__drizzle_migrations` 與 pin 的檔數、hermes 的 `_config_version` 與每份 config 只有一個 `platforms:`、tencentdb 有沒有 degraded + FTS marker 欄位 + 54 條 meta 路由對空 body 沒有 5xx; `scripts/upgrade.sh` 自己會跑它並在失敗時讓升版整個失敗)、`tests/scientist.sh` (live: 專家 lane 端到端)、`tests/podenv.sh` (live: 巢狀容器租約的結構 + 端到端)、`tests/devenv-s3.sh` (live: RustFS S3 bucket/IAM isolation, presigned, durability)、`tests/devenv-rabbitmq.sh` (live: RabbitMQ vhost/user isolation, limits, AMQP, durability)、`tests/memory-scope.sh` (live: frontdoor 記憶入境的結構 + 對抗回歸 —— 餵的是 `_meta` payload, 而且**同一份內容只要拿掉 `_meta` 就必須不進記憶**); 外加偶爾才跑的 `tests/fresh-install.sh` (乾淨機器完整排練, 見「部署假設」一節, 現在把 `tests/migrations.sh`、`tests/podenv.sh`、`tests/devenv-s3.sh`、`tests/devenv-rabbitmq.sh` 與 `tests/memory-scope.sh` 收進它的九個 gate 裡跑)。`tests/memory-ingress.sh` (離線: projector 的策略與對抗案例, 跑在 `tests/fixtures/buzz-acp-prompt-blocks.json` 這份凍結的 wire 契約上) 與 `tests/memory-ingress-meta.sh` (離線: 同一份契約的 Rust 產出端 —— scratch clone + `patch --fuzz=0` + `cargo test`) 是新增的兩條; `tests/outdated.sh`、`tests/upgrade-workflow.sh` 與 `tests/upgrade-preflight.sh` 是**離線**的 (在 fixture root 裡以假 `git`/`docker` 跑真的 script), 不需要活著的 stack 也不碰網路。這是慣例, 不是強制關卡 —— `scripts/prepare.sh` 不對它 gate。
