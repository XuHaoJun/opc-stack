...
- `tests/` — 測試與驗證腳本的慣例落腳處, 與 `scripts/` 分工: `scripts/` 是操作工具, `tests/` 是測試與驗證。移進來的檔案把冗餘的 `test-` 前綴拿掉 (目錄名已經說明性質了) —— 例外是 `tests/audit-bootstrap.sh` (本來就沒有前綴) 與 `tests/acp-smoke-test.mjs` (`-test` 是複合名稱的一部分而非前綴, 且它在 paperclip 容器內跑, 名稱與位置無關)。`tests/set-buzz-agent-owner.sh` 測的是 `scripts/set-buzz-agent-owner.sh` — 同名分屬 `scripts/`/`tests/` 兩個目錄是刻意的配對, 不是巧合, 其餘整合測試同理。七個 gate: `tests/audit-bootstrap.sh` (靜態: 每份狀態是否都有無人值守的產生者)、`tests/connectivity.sh` (live: 既有功能沒被弄壞)、`tests/migrations.sh` (live: 升版後 migration 有沒有真的前進 —— **這條存在的理由是 connectivity 在每一次無聲升版失敗裡都是綠的**: config ladder 吃掉 frontdoor 的 `system_prompt` 那次, 容器綠了 27 小時; 它比對 buzz `_sqlx_migrations` / paperclip `drizzle.__drizzle_migrations` 與 pin 的檔數、hermes 的 `_config_version` 與每份 config 只有一個 `platforms:`、tencentdb 有沒有 degraded + FTS marker 欄位 + 54 條 meta 路由對空 body 沒有 5xx; `scripts/upgrade.sh` 自己會跑它並在失敗時讓升版整個失敗)、`tests/scientist.sh` (live: 專家 lane 端到端)、`tests/podenv.sh` (live: 巢狀容器租約的結構 + 端到端)、`tests/devenv-s3.sh` (live: RustFS S3 bucket/IAM isolation, presigned, durability)、`tests/devenv-rabbitmq.sh` (live: RabbitMQ vhost/user isolation, limits, AMQP, durability); 外加偶爾才跑的 `tests/fresh-install.sh` (乾淨機器完整排練, 見「部署假設」一節, 現在把 `tests/migrations.sh`、`tests/podenv.sh`、`tests/devenv-s3.sh` 與 `tests/devenv-rabbitmq.sh` 收進它的七個 gate 裡跑)。`tests/outdated.sh`、`tests/upgrade-workflow.sh` 與 `tests/upgrade-preflight.sh` 是**離線**的 (在 fixture root 裡以假 `git`/`docker` 跑真的 script), 不需要活著的 stack 也不碰網路。這是慣例, 不是強制關卡 —— `scripts/prepare.sh` 不對它 gate。
